...
def confirm_action(*, user_id: str, message_id: str) -> BotActionResult:
    ...
    if _is_instagram_send_dm_action(action_type):
        return _confirm_instagram_send_dm_action(
            user_id=user_id,
            message_id=message_id,
            tool_calls=tool_calls,
            action_type=action_type,
...
def _confirm_instagram_send_dm_action(
    *,
    user_id: str,
    message_id: str,
    tool_calls: dict[str, Any],
    ...
    Mirrors _confirm_gmail_send_action's lifecycle:
      1. lock the proposal (confirm -> executing)
      2. resolve the peer id from the babyg thread row (never trust the
         payload alone — the payload carries the babyg thread uuid, not
         the peer ig id)
    ...
    (InstagramMessageWindowError surfaces as a distinct failure
    message the creator can act on: "the window closed, ask them to
    DM you first").
    """
    proposal_id = str(tool_calls.get("proposal_id") or "")
    if not proposal_id:
    ...
    result = _execute_instagram_send_dm(user_id=user_id, payload=payload)
    if result.get("ok"):
...
def _execute_instagram_send_dm(
    *, user_id: str, payload: dict[str, Any]
    ...
    thread_row = _load_ig_thread_for_send(
    ...
    try:
        ...
        logger.exception("bot._execute_instagram_send_dm.token user=%s", user_id)
    ...
    try:
        ...
        return {"ok": False, "reason": "outside_messaging_window", "detail": str(exc)[:200]}
    except instagram_meta.InstagramError as exc:
        logger.warning(
            "bot._execute_instagram_send_dm.send_failed user=%s error=%s",
            user_id,
            exc,
        )
        return {"ok": False, "reason": "instagram_send_failed", "detail": str(exc)[:200]}
    except Exception:
        logger.exception("bot._execute_instagram_send_dm.crashed user=%s", user_id)
        return {"ok": False, "reason": "instagram_send_failed"}
    return {"ok": True, "message_id": meta_message_id}


def _load_ig_thread_for_send(
    *, user_id: str, thread_uuid: str
    ...
    try:
        ...
        logger.exception(
            "bot._load_ig_thread_for_send.failed user=%s thread=%s",
            user_id,
            thread_uuid,
        )
        return None
...
def _is_instagram_send_dm_action(action_type: str) -> bool:
