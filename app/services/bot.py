...
def confirm_action(*, user_id: str, message_id: str) -> BotActionResult:
    ...
    if bot_ig_actions.is_instagram_send_dm_action(action_type):
        ig_result = bot_ig_actions.confirm_instagram_send_dm_action(
            user_id=user_id,
            message_id=message_id,
            tool_calls=tool_calls,
            action_type=action_type,
            update_message_tool_calls=_update_message_tool_calls,
            create_message=create_message,
        )
        return BotActionResult(
            message=ig_result.message,
            executed=ig_result.executed,
            action_type=ig_result.action_type,
            record_id=ig_result.record_id,
        )
...
def cancel_action(*, user_id: str, message_id: str) -> BotActionResult:
    ...
    if bot_ig_actions.is_instagram_send_dm_action(action_type):
        ig_result = bot_ig_actions.cancel_instagram_send_dm_action(
            user_id=user_id,
            message_id=message_id,
            tool_calls=tool_calls,
            action_type=action_type,
